@php
use App\Models\Translation;
$locale = app()->getLocale();

    $team = \App\Models\Team::where('is_active', 1)
        ->orderBy('sort')
        ->get();
@endphp

<section class="team-section">
    <div class="auto-container">
        <div class="sec-title text-center">
         <h2 class="section-title__title">
                {{ Translation::getValue('team_title', $locale) }}
            </h2>

            <div class="text">
                {{ Translation::getValue('team_description', $locale) }}
            </div>



        </div>

        <div class="row">
            @foreach($team as $member)
                <div class="team-block col-lg-3 col-md-6 col-sm-12">
                    <div class="image-box">
                        <figure class="image">
                            <img src="{{ asset('storage/' . $member->image) }}"
                                 alt="{{ $member->getName($locale) }}">
                        </figure>

                        <div class="overlay-box">
                            <div class="text">
                                {{ $member->getPosition($locale) }}
                            </div>

                            <ul class="social-icon">
                                @if($member->twitter)
                                    <li><a href="{{ $member->twitter }}"><span class="fab fa-twitter"></span></a></li>
                                @endif

                                @if($member->facebook)
                                    <li><a href="{{ $member->facebook }}"><span class="fab fa-facebook-f"></span></a></li>
                                @endif

                                @if($member->linkedin)
                                    <li><a href="{{ $member->linkedin }}"><span class="fab fa-linkedin-in"></span></a></li>
                                @endif
                            </ul>
                        </div>
                    </div>

                    <h4>
                        <a href="#">{{ $member->getName($locale) }}</a>
                    </h4>
                </div>
            @endforeach
        </div>
    </div>
</section>
